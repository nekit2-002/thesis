...
идентификаторе транзакции (Transaction ID),
которая породила историческую версию строки (или отдельных полей), сохраняемой в записи, и
указатель на запись с версией строки, которая существовала еще раньше RollPtr
...
Используя такие форматы записей, можно реализовать поддержку MVCC за счет них. Пусть мы имеем для каждой строки
в таблице идентификатор транзакции, которая последней вставляла или обновляла строку и указатель на запись

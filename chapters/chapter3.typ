...
Необходимые для работы табличного метода доступа интерфейсы описываются в документации,
приводимой в документе @docs. Согласно ей,
...
- _scan_begin_ --- функция для аллоцирования объекта--итератора, который используется системой для сканирования таблицы
...
- _tuple_satisfies_snapshot_ --- проверка на то, что кортеж, содержащийся в слоте, удовлетворяет условиям видимости
  заданного снимка
- _tuple_delete_ --- функция, отвечающая за удаление кортежа
- _tuple_update_ --- функция для обновления значений в полях кортежа
- _relation_estimate_size_ --- функция, позволяющая вычислить общий размер отношения (таблицы)



== Интеграция и процесс сборки расширения с PostgreSQL <integ>
...
Первый этап можно выполнить с помощью документации Postgres, приводимой в документе @docs.
Согласно ей, перед сборкой (на ОС Linux) необходимо
...
== Выводы
В процессе проектирования был описан общий вид архитектуры разрабатываемого расширения.
Разрабатываемый проект делится на две части: сам Postgres и директория с расширением,
в которой находятся файл контроля расширения и его исходный код.
Также были даны пояснения к тому, каким образом расширение будет встроено в сервер PostgreSQL.
Основное взаимодействие расширения с ядром сервера будет происходить через интерфейс
табличного метода доступа TableAmRoutine и его коллбеки. Интеграция расширения с сервером
проводится следующим образом:
- Собрать Postgres
- Установить фреймворк pgrx
- Связать pgrx-проект с сервером, указав при инициализации путь к control--файлу сервера
- Собрать расширение (установка расширения производится автоматически)

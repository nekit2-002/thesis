...
#figure(
  ...
  caption: [Настройки параметров autovacuum в экспериментах],
) <tbl:params>
...
Рассмотрим влияние такого бенчмарка на стандартный метод доступа в PostgreSQL @vanilla. При стандартных настройках autovacuum а задержке
(Latency) имеются пики значений, самое большое из которых достигает 50 миллисекунд. Тем не менее, смотря на график в целом, можно увидеть, что подавляющее
большинство этих пиков едва превышает значения в 26 миллисекунд, а самый большой является единственным выбросом. Что касается TPS,
его поведение можно разделить на 4 периода: 0 --- 50 сек., 50 --- 120 сек. и 120 -- 180 сек. и 180 --- 300 сек. В первом периоде происходит
активная вставка данных, но поскольку таблицы быстро распухают, TPS начинает стремительно снижаться. Во втором периоде
с приходом вакуума нагрузка на систему увеличивается и происходят резкие флуктуации TPS, который пока что держится на уровне от 10 000
...
#figure(image("../images/analysis/vanilla.png"), caption: [Эксперименты с autovacuum на стандартном Heap Access Method]) <vanilla>
...
#figure(image("../images/analysis/zheap.png"),
caption: [Эксперименты с autovacuum на zheap]) <zheap>
...
#figure(image("../images/analysis/oriole.png"),
caption: [Эксперименты с autovacuum на orioledb]) <oriole>

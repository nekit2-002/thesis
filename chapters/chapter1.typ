...
из--за распухания таблиц. Для этого можно либо использовать Foreign Data Wrapper (FDW) API, чтобы создать обертку сторонних данных, расположив их таким образом, чтобы ускорить запрос; либо использовать Table Access Method API, чтобы создать новый табличный метод доступа к данным,
...
Тем не менее, из опыта разработчиков данного проекта (компании Citus, на момент написания данной работы являющейся
частью Microsoft) известно, что хотя описанный метод хранения данных и имеет хорошую производительность относительно
...
расположения данных с помощью FDW показывают, что хотя это и не является неосуществимой задачей,
